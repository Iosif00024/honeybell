...
class AddressBuilder(QWidget):
    ...
    def create_keyboard(self):
        ...
        for row_number, characters in enumerate(self.KEYBOARD_ROWS):
            ...
            row.setContentsMargins(left_margin, 0, right_margin, 0)
            for character in characters:
                button = self.create_key(self.display_character(character), character=character)
        ...
        controls.addWidget(self.create_key("space", callback=partial(self.append_character, " "), width=160))
        controls.addWidget(self.create_key("x", callback=self.clear, width=76))
        controls.addWidget(self.create_key("→", callback=self.navigate_requested.emit, width=76))
    ...
    def update_hint(self):
        self.hint.setText(f"{self.address}")
...
class Browser(QMainWindow):
    def __init__(self, start_url):
        ...
        self.back = self.add_toolbar_button(self.toolbar, "←", self.go_back)
        self.forward = self.add_toolbar_button(self.toolbar, "→", self.go_forward)
        self.reload = self.add_toolbar_button(
            self.toolbar, "↻", 
            lambda: self.page().reload() if isinstance(self.page(), QWebEngineView) else None
        )
        self.add_toolbar_button(self.toolbar, "+", self.add_tab)
        self.address = QLineEdit(objectName="addressBar")
        self.address.setContextMenuPolicy(Qt.DefaultContextMenu) 
        self.address.returnPressed.connect(self.navigate)
        self.toolbar.addWidget(self.address)
        self.zoom_indicator = QPushButton(objectName="zoomLabel")
        self.zoom_indicator.setCursor(Qt.PointingHandCursor)
        self.zoom_indicator.setToolTip("Reset Zoom")
        self.zoom_indicator.clicked.connect(self.zoom_reset)
        self.zoom_indicator.hide()
        self.toolbar.addWidget(self.zoom_indicator)
        self.zoom_timer = QTimer(self)
        self.zoom_timer.setSingleShot(True)
        self.zoom_timer.timeout.connect(self.check_hide_zoom_indicator)
        self.add_toolbar_button(self.toolbar, "→", self.navigate)
        ...
        self.tabs.currentChanged.connect(self.update_address)
        ...
        zoom_in_alt.setContext(Qt.ApplicationShortcut)
        ...
        zoom_reset_shortcut.setContext(Qt.ApplicationShortcut)
    ...
    def show_zoom_indicator(self, factor):
        percentage = int(round(factor, 2) * 100)
        self.zoom_indicator.setText(f"{percentage}%")
        self.zoom_indicator.show()
        if percentage == 100:
            self.zoom_timer.start(2000)
        else:
            self.zoom_timer.stop()
    def check_hide_zoom_indicator(self):
        page = self.page()
        if isinstance(page, QWebEngineView):
            percentage = int(round(page.zoomFactor(), 2) * 100)
            if percentage == 100:
                self.zoom_indicator.hide()
        else:
            self.zoom_indicator.hide()
    def zoom_in(self):
        page = self.page()
        if isinstance(page, QWebEngineView):
            current_zoom = page.zoomFactor()
            new_zoom = min(current_zoom + 0.1, 3.0)
            page.setZoomFactor(new_zoom)
            self.show_zoom_indicator(new_zoom)
    def zoom_out(self):
        page = self.page()
        if isinstance(page, QWebEngineView):
            current_zoom = page.zoomFactor()
    ...
    def zoom_reset(self):
        ...
        if isinstance(page, QWebEngineView):
            ...
            self.show_zoom_indicator(1.0)
    def go_back(self):
        page = self.page()
        if isinstance(page, QWebEngineView) and page.history().canGoBack():
            page.back()
    def go_forward(self):
        page = self.page()
        if isinstance(page, QWebEngineView) and page.history().canGoForward():
            page.forward()
    ...
    def dropEvent(self, event):
        if event.mimeData().hasUrls():
            for url in event.mimeData().urls():
                self.add_web_tab(url.toString())
            event.acceptProposedAction()
        elif event.mimeData().hasText():
            text = event.mimeData().text().strip()
            if text:
                self.add_web_tab(text)
            event.acceptProposedAction()
        else:
    ...
    def check_autohide(self):
        ...
        if not self.toolbar.isVisible():
            if 0 <= pos.x() <= self.width() and 0 <= pos.y() <= 5:
                self.toolbar.show()
                self.tabs.tabBar().show()
        else:
            combined_height = self.toolbar.height() + self.tabs.tabBar().height()
            if pos.y() > combined_height or pos.x() < 0 or pos.x() > self.width() or pos.y() < 0:
                self.toolbar.hide()
    ...
    def add_tab(self, url="about:blank"):
        ...
        if url != "about:blank":
            tab.show_web_page()
            web_page.setUrl(QUrl.fromUserInput(url))
        return web_page
    def add_page_tab(self, builder, web_page, title=""):
    ...
    def create_web_page(self):
        ...
        page.setContextMenuPolicy(Qt.DefaultContextMenu)
        page.urlChanged.connect(self.update_address)
        page.titleChanged.connect(lambda title: self.rename_tab(page, title))
        page.loadFinished.connect(lambda: self.update_buttons(page))
        page.page().profile().downloadRequested.connect(self.handle_download)
        return page
    def handle_download(self, download):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Save download", os.path.join(folder, filename), "All files (*)"
        )
        if not path:
            download.cancel()
            return
    ...
    def install_close_button(self, tab, index):
        ...
        button.clicked.connect(lambda: self.close_tab(self.tabs.indexOf(tab)))
    ...
    def open_address(self, address):
        ...
        if isinstance(page, AddressBuilder):
            ...
            tab.show_web_page()
            tab.web_page.setUrl(QUrl.fromUserInput(address))
            self.update_buttons(tab.web_page)
        else:
            page.setUrl(QUrl.fromUserInput(address))
    ...
    def update_buttons(self, page):
        ...
        self.back.setEnabled(enabled and page.history().canGoBack())
        self.forward.setEnabled(enabled and page.history().canGoForward())
...
if __name__ == "__main__":
    ...
    browser = Browser(
        sys.argv[1] if len(sys.argv) > 1 else "about:blank"
    )
    browser.show()
    try:
        sys.exit(app.exec())
